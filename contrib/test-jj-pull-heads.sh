...
# Test script for jj-pull-heads.sh — pairs of jj repos in /tmp/, sync
# scenarios: fetch, abandon propagation, rewrites, working-copy
# protection, dry-run, ...
set -Eeuo pipefail #-x
...
(
  ...
  HEADS_BEFORE="$(heads_of "$T/local" | sort)"
  assert "dry-run lists fetch"            sh -c 'echo "$1" | grep -qi "will fetch"' sh "$OUT"
  assert "dry-run details import"         sh -c 'echo "$1" | grep -qi "as reachable heads"' sh "$OUT"
...
(
  ...
  printf '%s\n' "$OUT" >&2     # dump into the log for failure inspection
  assert "verbose mentions carried merge"    sh -c 'echo "$1" | grep -qi "carried-heads merge"' sh "$OUT"
  assert "verbose mentions discarded merge"   sh -c 'echo "$1" | grep -qi "hidden-heads merge"' sh "$OUT"
  assert "no pull bookmarks left" test -z "$(cd "$T/local" && jj bookmark list 2>/dev/null | grep __jj_pull || true)"
)

# ================= T15: non-colocated repos =================
# `jj git init` is colocated by default since jj 0.44, hence --no-colocate.
t15()
(
...
(
  ...
  cat > "$T/ssh" <<'EOSSH'
#!/bin/sh
# Fake ssh: run the remote command locally (git ssh transport included).
while test $# -gt 0 ; do
  case "$1" in
    -o|-p|-i) shift 2 ;;
    -*) shift ;;
...
EOSSH
  ...
  assert "A abandoned locally"        is_hidden  "$T/local" "$A"
  assert "C imported as head"         is_head    "$T/local" "$C"
  # round-trip: the remote only holds X as a raw git object, so a third
  # repo that never saw X must fetch it as a raw object
  X=$(mkcommit "$T/local" X)
...
(
  ...
  assert "verbose counts overlapping heads"  sh -c 'echo "$1" | grep -qi "overlapping head(s)"' sh "$VOUT"
  # new remote head: dry-run reports the planned imports/abandons but changes nothing
  C=$(mkcommit "$T/remote" C)
  HEADS_BEFORE="$(heads_of "$T/local" | sort)"
  OUT="$(cd "$T/local" && "$S" -n ../remote 2>&1)"
  printf '%s\n' "$OUT" >&2     # dump into the log for failure inspection
  assert "dry-run plans remote commits"        sh -c 'echo "$1" | grep -qi "will fetch .* remote commit(s)"' sh "$OUT"
  assert "dry-run plans abandonment pass"      sh -c 'echo "$1" | grep -qi "will abandon up to .* local head(s)"' sh "$OUT"
  assert "dry-run makes no changes"            test "$HEADS_BEFORE" = "$(heads_of "$T/local" | sort)"
  ( cd "$T/local" && sync ../remote )
  assert "C imported as head"               is_head   "$T/local" "$C"
...
(
  ...
  printf '%s\n' "$OUT" >&2                        # dump into the log for failure inspection
  assert "dry-run --keep notes no abandonment"    sh -c 'echo "$1" | grep -qi "no local heads will be abandoned"' sh "$OUT"
  assert "dry-run --keep omits abandon plan"      sh -c '! echo "$1" | grep -qi "will abandon up to"' sh "$OUT"
  assert "dry-run --keep still plans fetch"       sh -c 'echo "$1" | grep -qi "will fetch"' sh "$OUT"
...
(
  ...
  printf '%s\n' "$OUT" >&2
  assert "no heads either side message"   sh -c 'echo "$1" | grep -qi "nothing to sync (no heads on either side)"' sh "$OUT"
  ...
  OUT="$(cd "$T/local" && "$S" -r 'none()' ../remote 2>&1)"
  printf '%s\n' "$OUT" >&2
  assert "no remote/known message"         sh -c 'echo "$1" | grep -qi "nothing to sync (no remote heads, no known local heads)"' sh "$OUT"
